...
const react_basic = [
  ...
  },
  {
    ...
    content:
      "useCallback同useMemo使用方式类似，都是用于组件优化缓存，前者缓存的是一个函数，后者缓存的是函数的返回值。useCallback通常用于优化子组件，而useMemo可用于优化当前组件以及子组件。",
  },
];
...
const vue_basic = [
  {
    ...
    content:
      "共同点：都是提取公共函数逻辑复用。不同点：setup只会在组件实例初始化的时候执行一次，通过闭包的方式将引用挂载在组件内部；hooks则是在组件每次render的时候调用，隐式的挂载在当前组件实例内部的节点上，在下一次render的时候根据调用顺序取出来。",
  },
  {
    title: "vue响应式原理",
    content:
      "vue2是使用Object.definePeoproty数据拦截+事件订阅来实现的。在初始化的时候在内部把data里的数据转成一个个的get,set进行依赖收集，在数据发生变动的时候通知watcher来改变视图。",
  },
  {
    title: "router hash与router history的区别",
    content:
      "最明显的区别是hash模式url会带一个#号，而history则不会。hash模式利用的是#改变url不会触发页面的重新加载，但是却可以记录到浏览器的历史纪录，当#后面的url改变时，会触发onhashchange事件，用以触发页面的更新；而history模式则使用html5新增的两个api history.pushState和history.replaceState,push为新增一条记录，replace为替换当前记录，即使改变url也不会触发页面的重新加载。需要注意的是在history模式下刷新页面，会向后台发起请求，如果不存在则会报404，因此history模式下需要后台配置允许请求。",
  },
];
...
const build = [
  {
    ...
    content:
      "webpack开启服务需要先收集模块依赖，从入口文件开始，将其所依赖的js或者静态文件通过loader打包成一个文件，然后这个打包后的文件将从serve传递到浏览器运行。这样的处理规则，当修改一个文件时，整个包会重新构建一遍，这就是为啥改动一下需要10s+的反应时间浏览器才反应过来。且随着包的体积越来越大，更新速度也会越来越慢；而vite得益于es module加载模块特性，在开启服务使不需要先打包构建，只需收集相关模块依赖，将其转成es module模块加载",
  },
];
...
export default function (req, res) {
  ...
  } else return res.status(404).json({ message: "request method error" });
}
